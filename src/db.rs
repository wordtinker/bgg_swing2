...
pub fn get_unstable_games() -> Result<Vec<Game>, Error> {
    let conn = Connection::open(DB_FILE_NAME)?;
    let mut stmt = conn.prepare("select id, name, page, num_votes, rating from games where not stable order by random()")?;
    let iter = stmt
        .query_map(NO_PARAMS, |r| Game {
            id: r.get(0),
            name: r.get(1),
            page: r.get(2),
            votes: r.get(3),
...
impl DbConn {
    ...
    pub fn add_user(&self, user: &User, trusted: bool) -> Result<(), Error> {
        let now = Local::now();
        match self.conn.execute("insert or ignore into users (name, updated, trusted) values (?1, ?2, ?3)",
    ...
    pub fn check_user(&self, user: &User) -> Result<Option<bool>, Error> {
        let mut stmt = self.conn.prepare("select trusted from users where name = ?")?;
